...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.pieces) {
        const basePieces = parseInt(product.pieces, 10);
        setCalculatedPieces(`${basePieces * quantity}`);
      }
      if (product.serves) {
        const baseServes = parseInt(product.serves, 10);
        setCalculatedServes(`${baseServes * quantity}`);
      }
      if (product.nutrition) {
        setCalculatedNutrition({
          calories: (product.nutrition.calories * quantity).toFixed(2),
          protein: (product.nutrition.protein * quantity).toFixed(2),
          fat: (product.nutrition.fat * quantity).toFixed(2),
          omega3: product.nutrition.omega3 ? (product.nutrition.omega3 * quantity).toFixed(2) : undefined,
        });
      }
    }
  }, [quantity, product]);
